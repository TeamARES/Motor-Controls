...
s = socket.socket()
# host = '192.168.10.103'  #IP Address of the Raspberry pi
host = '192.168.29.92'
...
def sendDatatoRaspi():
    ...
    if(mode == 0):
        stringData = str(mode) + ',' + \
            str(forwardBackwardSpeed) + ',' + str(leftRightSpeed)
        s.send(str.encode(stringData))
    elif(mode == 1):
        stringData = str(mode) + ',' + str(baseMotor) + ',' + str(baseActuator) + ',' + str(
            armActuator) + ',' + str(clawPitch) + ',' + str(clawRoll) + ',' + str(clawOpenClose)
        s.send(str.encode(stringData))
...
def openBaseActuator():  # inc base actuator connected speed
    global baseActuator
    baseActuator = 100
    printSpeeds()
    sendDatatoRaspi()
    ...
    if format(key) == 'Key.up':
        ...
        if(len(keyData) == 4):  # If it is a number
            ...
            if(shotenKeyData == '1'):
                ...
                print('It is w')
                openBaseActuator()  # 100 (pins(6,13) NO PWM) - L298
            elif(shotenKeyData == 'd'):  # d
                print('It is d')
                incBaseMotorSpeed()  # increase the speed of __ (pins(25,24) PWM) - IBT2
            elif (shotenKeyData == 'a'):  # a
                print('It is a')
                decBaseMotorSpeed()  # decrease the speed of __ (pins(25,24) PWM) - IBT2
            elif (shotenKeyData == 's'):  # s
                print('It is s')
                closeBaseActuator()  # -100 pins(6,13)NO PWM - L298
            elif (shotenKeyData == 'r'):  # r
                print('It is r')
                decRollMotorSpeed()  # with PWM 21,20,16 - L298 (one is enable)
            elif (shotenKeyData == 't'):  # t
                print('It is t')
                incRollMotorSpeed()  # with PWM 21,20,16 - L298 (one is enable)
            elif (shotenKeyData == 'o'):  # o
                print('It is o')
                antiClockwisePitch()  # with PWM 12,1,10 - L298
            elif (shotenKeyData == 'p'):  # p
                print('It is p')
                clockwisePitch()  # with PWM 12,1,10 - L298
            elif (shotenKeyData == 'c'):  # c
                print('It is c')
                openCLaw() # no PWM Pin 7,8 - L298
            elif (shotenKeyData == 'v'):  # v
                print('It is v')
                closeClaw() ## no PWM Pin 7,8 - L298
            elif (shotenKeyData == 'z'):  # z
                print('It is z')
                closeArmActuator() # no PWM 19,26 - L298 
            elif (shotenKeyData == 'x'):  # x
                print('It is x')
                openArmActuator() ## no PWM 19,26 - L298
